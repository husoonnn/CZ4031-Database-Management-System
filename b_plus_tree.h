...
private:
  ...
  friend class BTree; // Let the BPlusTree class access this class' private variables.
...
private:
  // Variables
  MemPool *disk;     // Pointer to a memory pool for data blocks.
  MemPool *index;    // Pointer to a memory pool in disk for index.
...
public:
  ...
  BTree(std::size_t blockSize, MemoryPool *disk, MemoryPool *index);
